...
def build_system_prompt(companion: dict, context_logs: list[dict]) -> str:
    """Build the system prompt with companion persona and RAG context."""
    context_str = ""
    if context_logs:
        context_str = "\n".join(
            f"[{log['sender']}]: {log['message']}" for log in context_logs
        )
    else:
...
async def chat_websocket(websocket: WebSocket, companion_id: UUID):
    ...
    try:
        while True:
            ...
            system_prompt = build_system_prompt(companion, relevant_logs)
